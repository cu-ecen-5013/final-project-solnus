...
IOTHUBMESSAGE_DISPOSITION_RESULT receive_msg_callback(IOTHUB_MESSAGE_HANDLE message, void* user_context)
{
    ...
    if (content_type == IOTHUBMESSAGE_BYTEARRAY)
    {
        ...
        {
            (void)LOG("Failure retrieving byte array message\r\n");
        }
        else
        {
            ...
            (void)LOG("Received Binary message\r\n Data1: %.*s\r\n", (int)buff_len, (char*)user_context);
        }
    }
    else
    {
        ...
        else
        {
            ...
            (void)LOG("Received String Message\r\nData: %s\r\n", (char*)user_context);
        }
    }
...
int iothub_init(IOTHUB_DEVICE_CLIENT_LL_HANDLE *device_ll_handle)
{
    ...
    (void)IoTHub_Init();
    (void)LOG("Creating IoTHub Device handle\r\n");
    // Create the iothub handle here
    *device_ll_handle = IoTHubDeviceClient_LL_CreateFromConnectionString(connectionString, protocol);
    if (*device_ll_handle == NULL)
    {
        (void)LOG("Failure creating IotHub device. Hint: Check your connection string.\r\n");
        return 0;
    }
    LOG("IoT hub init successfully\r\n");
    return 1;
}
...
void iothub_RC_handler(IOTHUB_DEVICE_CLIENT_LL_HANDLE *device_ll_handle, void  *revStr){
    ...
    {
        (void)LOG("ERROR: IoTHubClient_LL_SetMessageCallback..........FAILED!\r\n");
        return ;
    }
}
